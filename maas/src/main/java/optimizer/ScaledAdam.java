...
public class ScaledAdam implements Optimizer{
	...
	public ScaledAdam(Plan maasAgentPlan) {
		...
		for(Entry<String, Object> a:maasAgentPlan.getAttributes().getAsMap().entrySet()){
			if(a.getValue() instanceof VariableDetails) {
				this.variables.put(a.getKey(), (VariableDetails)a.getValue());
			}
		}
		this.initialize();
	}

	public ScaledAdam(String id, Map<String,VariableDetails> variables) {
		this.variables = variables;
		this.id = id;
		this.initialize();
	}
	
	public ScaledAdam(String id, Map<String,VariableDetails> variables,double alpha,double beta1,double beta2,double eta) {
		this.variables = variables;
		this.alpha = alpha;
		this.beta1 = beta1;
		this.beta2 = beta2;
...
//		});
	...
	public Map<String,VariableDetails> takeStep(Map<String,Double> gradient){
		...
		RealVector p_real = m2a.getRealVector(this.variables.keySet().stream().collect(Collectors.toMap(k->k, k->this.variables.get(k).getCurrentValue())));
		RealVector p_min = m2a.getRealVector(this.variables.keySet().stream().collect(Collectors.toMap(k->k, k->this.variables.get(k).getLimit().getFirst())));
		RealVector p_max = m2a.getRealVector(this.variables.keySet().stream().collect(Collectors.toMap(k->k, k->this.variables.get(k).getLimit().getSecond())));
		RealVector p = p_real.subtract(p_min);
		...
		p_new = p_min.add(p_new.ebeMultiply(p_max.subtract(p_min)));
